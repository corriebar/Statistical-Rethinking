...
To summarize, getting the priors right and interpretating the parameters for a polynomial regression is really difficult.
...
d <- cherry_blossoms
plot( doy ~ temp, d, col=col.alpha(rangi2, 0.8), pch=20,
...
plot(d2$temp, d2$doy, col=col.alpha(rangi2, 0.6), pch=20, 
...
plot(doy ~ temp_s, data=d2, col=col.alpha(rangi2, 0.6),
...
plot(d2$temp, d2$doy, col=col.alpha(rangi2, 0.3), pch=20, 
     ylab  = "Day in Year", xlab = "Temperature [°C]")
shade( mu_PI, d2$temp)
shade( doy_PI, d2$temp)
lines(mu_mean ~ d2$temp)
...
The splines are wigglier and have one especially weird dip just for temperatures below 6°C. Personally, I am sceptical this dip makes physically sense and it isn't just an artifact of the splines being too wiggly. Comparing the three models, in this case, I'd go with the simple linear model.
...
This alsmost worked, except that we now have that the splines seem to come from far down to the left. This is probably since we specified that the weights are distributed around 0: $w \sim \text{Normal}(0, 10)$.
...
<small>[Full code.](https://github.com/corriebar/Statistical-Rethinking/blob/master/Chapter_4/chapter4_Ex.Rmd)<small>
