...
library(tidyverse)
```
These are code snippets and a short summary of the fourth chapter, _Geocentric Models_, of the book "Statistical Rethinking" (version 2) by Richard McElreath.
...
There are boradly two reasons why we're using Gaussian distributions in modelling:
...
```{r, echo=F}
precis(d) %>%
  knitr::kable("html") %>% kableExtra::kable_styling(full_width = F, position = "center")
```
...
Posterior of $\sigma$ tend to have a long-right hand tail. The reasons? Complex! But an intution is that, because the standard deviation has to be positive, there is less uncertainty how small it is (it is bounded by 0 after all) and more uncertainty about how big it is.
If we repeat the example from above on a small subset of the data, this tail in the uncertainty  $\sigma$ becomes more pronounced:
```{r, echo=F, fig.height=5, fig.width=5}
...
m4.2 <- quap( 
          ...
          data=d2)
precis(m4.2) %>% knitr::kable("html") %>% kableExtra::kable_styling(full_width = F)
```
...
```
```{r, echo=F}
precis(post) %>% knitr::kable("html") %>% kableExtra::kable_styling(full_width = F)
```
