...
### Visualize our Model
Let's visualize the model to get a better understanding of its results. We start with plotting the raw data and the posterior mean of $\alpha$ and $\beta$ as a kind of "mean line":
...
We could also simulate the heights manually. Remember, to manually compute $\mu$ (instead of using `link()`), we can use the following:
```{r}
post <- extract.samples(m4.3)
weight.seq <- 25:70
